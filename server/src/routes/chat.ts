...
function buildSystemPrompt(
  ...
  let base =
    CHAT_SYSTEM_PROMPT +
    '\n\nResponse style rules (apply to every reply): Keep default answers concise at 3-5 sentences. For simple numeric/fact questions, answer in 1-2 short sentences and put the key number/fact first. For explanation questions (how/why/compare/strategy), keep to 4-7 sentences unless the user explicitly asks for more detail. Use plain text only: do not use markdown formatting such as **bold**, headers, or bullet/numbered lists. End with at most one short follow-up offer only when useful.' +
    ...
    '\n\nFor mortgage or general questions that do not require search_listings, follow the response style rules above and keep answers practical and direct.';
  if (mode === 'guided') {
    base +=
